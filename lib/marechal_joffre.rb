...
def search_hashtag(client, query)
  client.search(query, count: 25).take(25)
end

def like_tweets(client, arr)
  client.favorite(*arr)
end

def get_user(arr)
  out = []
  arr.each {|tweet| out << tweet.user}
  return out
end

def follow_users(client, arr)
  uni_arr = arr.uniq.reject {|u| u == myself(client) || u == u.following?}
  client.follow(*uni_arr)
end

def follow_one_user(client, usr)
  if usr.following? || usr == myself(client)
    nil
  else
    client.follow(usr)
  end
...
s = stream_twitter
s.filter(track: "#bonjour_monde") do |object|
  if object.is_a?(Twitter::Tweet)
    c = login_twitter
    puts object.full_text
    like_tweets(c, object)
    puts
    puts "Tweet liked!"
    us = object.user
    follow_one_user(c, us)
    puts
    puts "User followed!"
  end
end
